...
# Usage :
# bash c14-get-old-archive.sh [dev or nothing]
...
# If the returned archive's array is not empty and bigger than the archivesRetention variable
if [ ${#archivesDate[@]} -ne 0 ] && (( ${#archivesDate[@]} > $archivesRetention )); then
...
if [ ${#archivesDate[@]} -ne 0 ] && (( ${#archivesDate[@]} > $archivesRetention )); then
    ...
    # Set oldest archive's informations in the scope
    ...
    if [[ $env = "dev" ]]; then
        echo `date +%Y-%m-%d_%H:%M:%S`" : The archive to delete is $archiveId"
    fi
    ...
    if [[ $env = "dev" ]]; then
        echo `date +%Y-%m-%d_%H:%M:%S`" : No archive to delete was found for the $env environment"
    fi
